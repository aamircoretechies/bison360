...
function AccordionMenuLabel({ children, className, ...props }: AccordionMenuLabelProps) {
  ...
  return (
    <div
      ...
      role="presentation"
      className={cn('px-2 py-1.5 text-xs font-medium text-[#F7E8D370]', classNames?.label, className)}
      {...props}
    >
      {children}
    </div>
      ...
      >
        <>
          ...
          <ChevronDown
            ...
            className={cn(
              'ms-auto size-3.5! shrink-0 text-[#F7E8D370] transition-transform duration-200 [[data-state=open]>&]:-rotate-180',
            )}
          />
        </>
      </AccordionPrimitive.Trigger>
